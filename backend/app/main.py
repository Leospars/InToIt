...
async def generate_quiz(topic: str, difficulty: str = "medium", num_questions: int = 5):
    ...
    prompt = f"Generate a {difficulty} difficulty quiz about {topic} with {num_questions} questions. Follow this JSON format: {quiz_format}"
...
async def generate_flashcards(topic: str, num_cards: int = 10):
    # Prompt gemini
    client = genai.Client(api_key=settings.gemini_api_key)
    ...
    response = client.models.generate_content(
        model="gemini-3-flash-preview",
        contents=f"Generate {num_cards} flashcards about {topic}. Format as JSON: {flashcards_format}",
        output_mime_type="application/json",
    )
...
async def explain(topic: str, difficulty: str = "simple"):
    ...
    response = client.models.generate_content(
        model="gemini-3-flash-preview",
        contents=f"Explain {topic} in a {difficulty} way. Use clear steps and analogies.",
    )
